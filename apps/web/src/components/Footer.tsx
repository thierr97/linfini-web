...
const SOCIALS = [
  ...
  {
    name: 'Facebook',
    href: 'https://www.facebook.com/linfini.gp',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
  ...
  {
    name: 'TikTok',
    href: 'https://www.tiktok.com/@infinievents.gp',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
...
export default function Footer() {
  return (
    <footer className="bg-charbon border-t border-white/5">
      {/* CTA band */}
      <div className="bg-gradient-to-r from-braise/20 via-charbon to-ambre/10 border-b border-white/5 py-10 px-4">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="font-display text-2xl font-bold text-creme mb-1">Organiser un événement privé ?</h3>
      ...
      <div className="py-16 px-4">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="md:col-span-2">
            <h3 className="font-display text-3xl font-bold text-or mb-3">L&apos;Infini</h3>
            <p className="text-white/40 text-sm leading-relaxed mb-6 max-w-xs">
              Restaurant • Bar Lounge • Salle événementielle<br />
              Le complexe nocturne premium de Guadeloupe.
            </p>
            {/* Social icons */}
            <div className="flex items-center gap-3">
              {SOCIALS.map(s => (
                <a
                  ...
                  aria-label={s.name}
                  className="w-10 h-10 rounded-full border border-white/10 hover:border-braise/50 bg-white/5 hover:bg-braise/10 flex items-center justify-center text-white/40 hover:text-white transition-all"
                >
                  {s.icon}
                </a>
              ))}
